...
class App extends Component {
  ...
  inputDisplay = () => {
    ...
    } else if (this.state.secondArgument && this.state.firstArgument > 0) {
      display = this.state.secondArgument;
    } else display = this.state.firstArgument;
    return display;
  };
              ...
              {this.state.operators.map((operator, index) => {
                return (
                  <Operator
                    operator={operator}
                    key={index}
                    click={this.clickOperatorHandler}
                    style={this.buttonStyle}
                  />
                );
              })}
              <button style={this.buttonStyle} onClick={this.clickClearHandler}>
                C
              </button>
            </div>
          </div>
